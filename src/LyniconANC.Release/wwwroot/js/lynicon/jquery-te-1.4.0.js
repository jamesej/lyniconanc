...
(function($){		
	$.fn.jqte = function(options){
		...
		return this.each(function(){
			...
			linkbutton.after("<input type=\"checkbox\" class=\"" + vars.css + "_opentype\"/><span>New Window</span> <input type=\"checkbox\" class=\"" + vars.css + "_followtype\"/><span>Follow</span>");

			var linkopencheck = linkform.find("."+vars.css+"_opentype");
			var linkfollowcheck = linkform.find("."+vars.css+"_followtype");
				...
				if(!toolbar.data("sourceOpened"))
				{
					...
					if(selectedTag)
					{
						...
						if(thisTagName == "a" && selectedTag.is("[href]"))
						{
						    ...
						    followLink = (selectedTag.attr("rel") != "nofollow");
						    targBlankLink = (selectedTag.attr("target") == "_blank");
					...
					linkopencheck.prop("checked", targBlankLink);
					linkfollowcheck.prop("checked", followLink);
